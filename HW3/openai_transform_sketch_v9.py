#!/usr/bin/env python
"""Transform input photos into line-sketch style images using OpenAI Images API (v9)."""
...
DEFAULT_PROMPT_TEMPLATE = (
    ...
    "기존 선화, 구도, 원근, 건물과 나무의 형태는 유지해줘. "
    "직접적인 해 원판은 절대 그리지 말고, {sun_event} 직전 또는 직후의 분위기만 표현해줘. "
    "하늘과 지평선 근처, 건물 가장자리, 나무 끝에 은은한 붉은 기운과 희미한 빛이 스며드는 정도로만 보여줘. "
    "태양처럼 읽히는 동그란 형태, 강한 광원, 붉은 구슬, 붉은 원형 하이라이트는 만들지 마. "
    "전체적으로 차분하고 조용한 분위기여야 하고, 검은 선 스케치는 크게 바꾸지 말고 붉은 빛 기운만 최소한으로 얹어줘. "
    "다른 색은 쓰지 마."
)
DEFAULT_SUNRISE_PROMPT_TEMPLATE = (
    "이 이미지는 이미 완성된 흑백 단일 펜 미니멀 라인 스케치야. "
    "기존 선화, 구도, 원근, 건물과 나무의 형태는 유지해줘. "
    "직접적인 해 원판은 절대 그리지 말고, 해가 막 떠오르기 직전 순간의 분위기만 표현해줘. "
    "하늘이 아주 조금 밝아지고, 지평선 근처와 건물 가장자리, 나무 끝에 은은한 붉은 기운과 희미한 새벽빛이 스며드는 정도로만 보여줘. "
    "태양처럼 읽히는 동그란 형태, 강한 광원, 붉은 구슬, 붉은 원형 하이라이트는 만들지 마. "
    "전체적으로 차분하고 조용한 dawn 직전의 느낌이어야 하고, 검은 선 스케치는 크게 바꾸지 말고 붉은 빛 기운만 최소한으로 얹어줘. "
    "다른 색은 쓰지 마."
)
DEFAULT_SUNSET_PROMPT_TEMPLATE = (
    "이 이미지는 이미 완성된 흑백 단일 펜 미니멀 라인 스케치야. "
    "기존 선화, 구도, 원근, 건물과 나무의 형태는 유지해줘. "
    "직접적인 해 원판은 절대 그리지 말고, 해가 막 지고 난 직후 순간의 분위기만 표현해줘. "
    "하늘과 지평선 근처, 건물 가장자리, 나무 끝에 은은한 붉은 여광과 희미한 저녁빛이 남아 있는 정도로만 보여줘. "
    "태양처럼 읽히는 동그란 형태, 강한 광원, 붉은 구슬, 붉은 원형 하이라이트는 만들지 마. "
    "전체적으로 차분하고 조용한 dusk 직후의 느낌이어야 하고, 검은 선 스케치는 크게 바꾸지 말고 붉은 빛 기운만 최소한으로 얹어줘. "
    "다른 색은 쓰지 마."
)
...
def _prompt_for_image(
    *,
    ...
    if fixed_prompt:
        return fixed_prompt
    normalized_event = sun_event.lower()
    if prompt_template == DEFAULT_PROMPT_TEMPLATE:
        if normalized_event == "sunrise":
            return DEFAULT_SUNRISE_PROMPT_TEMPLATE
        if normalized_event == "sunset":
            return DEFAULT_SUNSET_PROMPT_TEMPLATE
    event_ko = {"sunrise": "일출", "sunset": "일몰"}.get(normalized_event, sun_event)
